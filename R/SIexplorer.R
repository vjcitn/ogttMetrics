...
#
 ...
 server = function(input, output, session) {
    ...
    output$sitext = renderText(paste("SI =", round(assay(oc)$SI[1, input$idpick], 6)))
    output$sivmat = renderPlotly( {
       newdf = data.frame(id=colnames(oc)[[1]], 
                   mats120=as.numeric(assay(oc)$Mats120),
                   SI=as.numeric(assay(oc)$SI[1,]),
                   converged=as.logical(as.numeric(assay(oc)$SI[2,])))
